...
export async function POST(request: NextRequest) {
  try {
    ...
    const { allowed, retryAfterMs } = checkRateLimit(`chat:${ip}`);
    if (!allowed) {
    ...
    // Global budget check
    const globalQuota = checkGlobalQuota('chat', Math.min(charCount, 5000));
    ...
    // Per-IP daily quota
    const quota = checkDailyQuota('chat', ip, Math.min(charCount, 5000));
    if (!quota.allowed) {
      return NextResponse.json(
