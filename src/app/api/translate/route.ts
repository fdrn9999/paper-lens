...
export async function POST(request: NextRequest) {
    try {
      ...
      const { allowed, retryAfterMs } = await checkRateLimit(`translate:${ip}`);
          if (!allowed) {
      ...
      const globalQuota = await checkGlobalQuota('translate', charCount);
      ...
      const quota = await checkDailyQuota('translate', ip, charCount);
          if (!quota.allowed) {
                  return NextResponse.json(
