  ...
  return (
    <div className="h-[100dvh] flex flex-col overflow-hidden max-w-[100vw]">
      ...
      <header className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 px-3 sm:px-4 py-2 sm:py-3 border-b bg-white shadow-sm z-40 shrink-0">
        ...
        <div className="flex items-center gap-1 sm:gap-2 sm:shrink-0 min-w-0">
          ...
          <div className="sm:hidden flex items-center gap-1">
            {pdfFile && (
              <button
                onClick={() => startGuide()}
                className="min-w-[44px] min-h-[44px] flex items-center justify-center rounded hover:bg-gray-100 text-gray-500 hover:text-gray-700 transition-colors shrink-0"
                title="가이드 다시보기"
                aria-label="가이드 다시보기"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="hidden sm:flex items-center gap-1 shrink-0">
          {pdfFile && (
            <button
              onClick={() => startGuide()}
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded text-sm text-gray-600 hover:bg-gray-100 hover:text-gray-800 transition-colors shrink-0"
              title="가이드 다시보기"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden relative">
        ...
        {/* Main - PDF Viewer */}
        <main id="main-content" className="flex-1 min-w-0 flex flex-col min-h-0" data-guide="pdf-viewer">
          <ErrorBoundary
            section="PDF 뷰어"
            fallback={
